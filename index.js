...
/**
 * Функция удаления карточки и данных пользователя
 * @param {HTMLDivElement} userCard - карточка пользователя
 * @param email - email удаляемого пользователя
 */
function deleteCard(userCard, email) {
...
/**
 * Функция удаления карточки и данных пользователя
 * @param email - email изменяемого пользователя
 */
function changeUser(email) {
    const currentName = document.querySelector('#name')
    const currentSecondName = document.querySelector('#secondName')
    ...
    currentSecondName.value = data.secondName
 ...
 * @param {string} data.city - email пользователя
        ...
        <div data-user=${email} class="user-outer">
            ...
            <div class="menu">
                ...
                <button data-change-user-email=${email} class="change">
                    <img src="./icons/gear.svg" alt="Изменить">
                </button>
            </div>
        </div>
    `
...
function addCard(e) {
    ...
    if (!newEmail.value || !newName.value || !newSecondName.value) {
        resetInputs(newName, newSecondName, newEmail, newCity)
        return
    }
    ...
    const data = {
        ...
        city: newCity.value === undefined ? '' : newCity.value,
    }

    const newCard = !newEmail.value
